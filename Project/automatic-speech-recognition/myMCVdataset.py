...
class myMozillaCommonVoiceDataset(torch.utils.data.Dataset):
    ...
    Args:
        ...
        csvfile (str, optional):
            The name of the csv file used to construct the metadata (default: ``"my-recorded-data.csv"``)
    """
    ...
    _ext_txt = ".txt"
    _ext_audio = ".wav"
    _folder_audio = "audio_files"

    def __init__(self, root: Union[str, Path], csvfile: str="my-recorded-data.csv", augment: bool=False, resample_audio: bool=False) -> None:
        ...
        self._csv = os.path.join(self._path, csvfile)

        with open(self._csv, "r") as csv_:
            walker = csv.reader(csv_, delimiter="\t")
    ...
    def __getitem__(self, n: int) -> Tuple[torch.Tensor, Dict[str, str]]:
        ...
        Returns:
            ...
            is built from the csv file with the following keys: ``client_id``, ``path``, ``sentence``,
