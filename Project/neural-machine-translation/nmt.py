...
NUM_EPOCHS = 16
NUM_DATA_SAMPLES_TO_USE = 80000
LOAD_PREV_SAVED_DATA_BATCHES = False


LOAD_PREV_MODEL_PARAMS_FROM_FILE = True
ENCODER_PARAMS_LOAD_FILE_NAME = 'encoder.pth'
DECODER_PARAMS_LOAD_FILE_NAME = 'decoder.pth'
ENCODER_PARAMS_SAVE_FILE_NAME = ENCODER_PARAMS_LOAD_FILE_NAME
DECODER_PARAMS_SAVE_FILE_NAME = DECODER_PARAMS_LOAD_FILE_NAME
...
# print(data.shape)
num_data = NUM_DATA_SAMPLES_TO_USE
data_x = data[0:num_data, 0]
data_y = data[0:num_data, 1]
numData = len(data_x)
...
print('finished transforming targets...')
...
print('finished prepending and postpending <> signal characters...')
print(f'it took {(time.time()-st):.1f} seconds to prepare data...')
...
if (LOAD_PREV_MODEL_PARAMS_FROM_FILE):
    encoder.load_state_dict(torch.load(path + 'model-params/' + ENCODER_PARAMS_LOAD_FILE_NAME))
    decoder.load_state_dict(torch.load(path + 'model-params/' + DECODER_PARAMS_LOAD_FILE_NAME))
...
def train_batch(input_batch, target_batch, encoder, decoder,
    ...
    enc_h_hidden = enc_h_hidden.to(device)
    enc_c_hidden = enc_c_hidden.to(device)
    ...
    for i in range(target_batch.shape[0]):
        ...
        dec_h_hidden = dec_h_hidden.to(device)
        dec_c_hidden = dec_c_hidden.to(device)
        ...
        p = pred.to('cpu').float()
        tb = target_batch[i].to('cpu').type(torch.LongTensor)
...
for i in range(epochs):
    ...
    print(f'epoch time: {(time.time()-ep_st):.1f} seconds')

print(f'\ntotal elapsed time: {(time.time()-st):.1f} seconds\n')
...
encoder_file_name = ENCODER_PARAMS_SAVE_FILE_NAME
decoder_file_name = DECODER_PARAMS_SAVE_FILE_NAME
torch.save(encoder.state_dict(), path + 'model-params/' + encoder_file_name)
torch.save(decoder.state_dict(), path + 'model-params/' + decoder_file_name)
